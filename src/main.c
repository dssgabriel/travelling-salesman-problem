...
/// Stores the final solution i.e., the path of the salesman.
...
void branch_and_bound(config_t const* config, int64_t curr_bound,
                      ...
                      int64_t* curr_path)
{
    // Base case: when we have reached the last level, which means we have
    // covered all the nodes.
    if (level == N) {
        // Check if there is an edge from last vertex in path back to the first
        // vertex.
    ...
    for (size_t i = 0; i < config->nb_nodes; i++) {
        ...
        if (current != 0 && visited[i] == false) {
            ...
            // Different computation of `curr_bound` for level 2 than for the
            // other levels.
            if (level == 1) {
...
void TSP(config_t* const config)
{
    ...
    // For all edges, computes the initial lower bound for the root node using
    // the following formula: 
    //     1/2 * (sum of first min + second min)
    // Also initializes the `curr_path` and `visited` array.
    ...
    // Call to `branch_and_bound` for `curr_weight` equal to 0 and level 1.
